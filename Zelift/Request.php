<?php
	...
	class Request extends SymfonyRequest
	{
		...
		/**
		 * The Thin session store implementation.
		 *
		 * @var \Thin\Session\Store
		 */
		...
		{
			return arrayGet($this->segments(), $index - 1, $default);
		}
		...
		{
			$boolOrArray = is_bool($this->input($key)) || Arrays::is($this->input($key));
		...
		{
			$input = $this->getInputSource()->all() + $this->query->all();dd($input);
		...
		{
			return arrayGet($this->files->all(), $key, $default);
		}
		...
		{
			if (!Arrays::is($files = $this->file($key))) {
				$files = array($files);
			}
		...
		/**
		 * Create an Thin request from a Symfony instance.
		 *
		 * @param  \Symfony\Component\HttpFoundation\Request  $request
		 * @return \Thin\Http\Request
		 */
