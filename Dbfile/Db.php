<?php
    ...
    class Db
    {
        ...
        public function model($data = [])
        {
            ...
            $modelFile = APPLICATION_PATH . DS . 'models' . DS . 'File' . DS . 'models' . DS . Inflector::lower($db) . DS . ucfirst(Inflector::lower($table)) . '.php';

            if (!is_dir(APPLICATION_PATH . DS . 'models' . DS . 'File')) {
                File::mkdir(APPLICATION_PATH . DS . 'models' . DS . 'File');
            }

            if (!is_dir(APPLICATION_PATH . DS . 'models' . DS . 'File' . DS . 'models')) {
                File::mkdir(APPLICATION_PATH . DS . 'models' . DS . 'File' . DS . 'models');
            }

            if (!is_dir(APPLICATION_PATH . DS . 'models' . DS . 'File' . DS . 'models' . DS . Inflector::lower($db))) {
                File::mkdir(APPLICATION_PATH . DS . 'models' . DS . 'File' . DS . 'models' . DS . Inflector::lower($db));
            }

            if (!File::exists($modelFile)) {
                File::put($modelFile, str_replace('##class##', ucfirst(Inflector::lower($db)) . ucfirst(Inflector::lower($table)) . 'ModelFile', File::read(__DIR__ . DS . 'dbModel.tpl')));
            }

            $class = '\\Thin\\' . ucfirst(Inflector::lower($db)) . ucfirst(Inflector::lower($table)) . 'ModelFile';
        ...
        {
            $file = $this->getFile('coreage');
            File::delete($file);
            File::put($file, (int) $age);
        ...
        public function delete($id)
        {
            ...
            $datas = unserialize(File::read($file));
        ...
        {
            if (empty($this->wheres)) {
                return count($this->getFiles('fields.id'));
            }
        ...
        public function all($object = false)
        {
            ...
            $ids = $this->getFiles('fields.id');

            foreach ($ids as $idFile) {
                $id = (int) str_replace('.db', '', Arrays::last(explode('/', $idFile)));
                $this->results[] = ['id' => $id];
            }
        ...
        public function exec($object = false, $first = false)
        {
            ...
            foreach ($this->results as $exists => $row) {
                if (false !== $exists) {
                    $data = File::read($this->getFile($exists));
            ...
            foreach ($sortable as $sort) {
                if (!empty($this->selects) && false === $object) {
                    ...
                    $item = unserialize(File::read($this->getFile($sort['id'])));
        ...
        {
            $has = Instance::has('fileDbCache', sha1($this->collection));

            if (true === $has) {
                return Instance::get('fileDbCache', sha1($this->collection));
            } else {
                $instance = Caching::instance($this->collection);

                return Instance::make('fileDbCache', sha1($this->collection), $instance);
            }
        }
        ...
        private function facade()
        {
            ...
            if (!class_exists($class)) {
                $code = 'namespace Dbfile; class ' . $facade . ' extends Facade { public static $database = "' . $this->db . '"; public static $table = "' . $this->table . '"; }';

                eval($code);

                Alias::facade('Dbfx' . $facade, $facade, 'Dbfile');
            }

            if (false !== $facade2) {
                $class2 = '\\Dbfile\\' . $facade2;

                if (!class_exists($class2)) {
                    $code2 = 'namespace Dbfile; class ' . $facade2 . ' extends Facade { public static $database = "' . $this->db . '"; public static $table = "' . $this->table . '"; }';

                    eval($code2);

                    Alias::facade('Dbfx' . $facade2, $facade2, 'Dbfile');
                }
            }
        ...
        public function min($field, $returRow = false, $object = false)
        {
            ...
            if (true === $this->useCache) {
                $this->cache()->set($keyData, [$rowId, $min]);
                $this->cache()->set($keyAge, time());
            }
        ...
        public function max($field, $returRow = false, $object = false)
        {
            ...
            if (true === $this->useCache) {
                $this->cache()->set($keyData, [$rowId, $max]);
                $this->cache()->set($keyAge, time());
            }
