<?php
    ...
    class Db
    {
        ...
        public function model($data = [])
        {
            ...
            $modelFile = APPLICATION_PATH . DS . 'models' . DS . 'Dbsql' . DS . 'models' . DS . Inflector::lower($db) . DS . ucfirst(Inflector::lower($table)) . '.php';

            if (!is_dir(APPLICATION_PATH . DS . 'models' . DS . 'Dbsql')) {
                File::mkdir(APPLICATION_PATH . DS . 'models' . DS . 'Dbsql');
            }

            if (!is_dir(APPLICATION_PATH . DS . 'models' . DS . 'Dbsql' . DS . 'models')) {
                File::mkdir(APPLICATION_PATH . DS . 'models' . DS . 'Dbsql' . DS . 'models');
            }

            if (!is_dir(APPLICATION_PATH . DS . 'models' . DS . 'Dbsql' . DS . 'models' . DS . Inflector::lower($db))) {
                File::mkdir(APPLICATION_PATH . DS . 'models' . DS . 'Dbsql' . DS . 'models' . DS . Inflector::lower($db));
            }

            if (!File::exists($modelFile)) {
                File::put($modelFile, str_replace('##class##', ucfirst(Inflector::lower($db)) . ucfirst(Inflector::lower($table)) . 'ModelSQL', File::read(__DIR__ . DS . 'dbModel.tpl')));
            }

            $class = '\\Thin\\' . ucfirst(Inflector::lower($db)) . ucfirst(Inflector::lower($table)) . 'ModelSQL';
            ...
            if (!$check) {
                if (!empty($condition)) {
                    ...
                    if (true === $this->useCache) {
                        $keyAge     = 'sql.db.where.age.' . $this->collection . '.' . sha1(serialize($condition));
                        $keyData    = 'sql.db.where.data.' . $this->collection . '.' . sha1(serialize($condition));
                    ...
                    if (empty($this->wheres)) {
                        ...
                        if (true === $this->useCache) {
                            $keyAge     = 'sql.db.where.make.age.' . $this->collection . '.' . sha1(serialize(array_keys($this->wheres)) . serialize($condition));
                            $keyData    = 'sql.db.where.make.data.' . $this->collection . '.' . sha1(serialize(array_keys($this->wheres)) . serialize($condition));
        ...
        public function exec3($object = false, $first = false)
        {
            if ($first) {
                $this->limit = 1;
            }
            ...
            if (true === $this->useCache && false === $object) {
                $keyData    = 'sql.db.run.data.' . $this->collection . '.' . $hash;
                $keyAge     = 'sql.db.run.age.' . $this->collection . '.' . $hash;
        ...
        private function facade()
        {
            ...
            if (!class_exists($class)) {
                $code = 'namespace Dbsql; class ' . $facade . ' extends Facade { public static $database = "' . $this->db . '"; public static $table = "' . $this->table . '"; }';

                eval($code);

                Alias::facade('Dbql' . $facade, $facade, 'Dbsql');
            }

            if (false !== $facade2) {
                $class2 = '\\Dbsql\\' . $facade2;

                if (!class_exists($class2)) {
                    $code2 = 'namespace Dbsql; class ' . $facade2 . ' extends Facade { public static $database = "' . $this->db . '"; public static $table = "' . $this->table . '"; }';

                    eval($code2);

                    Alias::facade('Dbql' . $facade2, $facade2, 'Dbsql');
                }
            }
        ...
        public function sum($field)
        {
            ...
            $keyData    = 'sql.db.sum.data.' . $this->collection . '.' . $hash;
            $keyAge     = 'sql.db.sum.age.' . $this->collection . '.' . $hash;
        ...
        public function min($field, $returRow = false, $object = false)
        {
            ...
            $keyData    = 'sql.db.min.data.' . $this->collection . '.' . $hash;
            $keyAge     = 'sql.db.min.age.' . $this->collection . '.' . $hash;
        ...
        public function max($field, $returRow = false, $object = false)
        {
            ...
            $keyData    = 'sql.db.max.data.' . $this->collection . '.' . $hash;
            $keyAge     = 'sql.db.max.age.' . $this->collection . '.' . $hash;
