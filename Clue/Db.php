<?php
    ...
    class Db
    {
        ...
        public function model($data = [])
        {
            ...
            $modelFile = APPLICATION_PATH . DS . 'models' . DS . 'Clue' . DS . 'models' . DS . Inflector::lower($db) . DS . ucfirst(Inflector::lower($table)) . '.php';

            if (!is_dir(APPLICATION_PATH . DS . 'models' . DS . 'Clue')) {
                File::mkdir(APPLICATION_PATH . DS . 'models' . DS . 'Clue');
            }

            if (!is_dir(APPLICATION_PATH . DS . 'models' . DS . 'Clue' . DS . 'models')) {
                File::mkdir(APPLICATION_PATH . DS . 'models' . DS . 'Clue' . DS . 'models');
            }

            if (!is_dir(APPLICATION_PATH . DS . 'models' . DS . 'Clue' . DS . 'models' . DS . Inflector::lower($db))) {
                File::mkdir(APPLICATION_PATH . DS . 'models' . DS . 'Clue' . DS . 'models' . DS . Inflector::lower($db));
            }

            if (!File::exists($modelFile)) {
                File::put($modelFile, str_replace('##class##', ucfirst(Inflector::lower($db)) . ucfirst(Inflector::lower($table)) . 'ModelClue', File::read(__DIR__ . DS . 'dbModel.tpl')));
            }

            $class = '\\Thin\\' . ucfirst(Inflector::lower($db)) . ucfirst(Inflector::lower($table)) . 'ModelClue';
        ...
        public function where($condition = [], $op = 'AND')
        {
            ...
            if (!$check) {
                if (!empty($condition)) {
                    ...
                    if (redis()->exists($cursor)) {
                        $ageCursor  = redis()->get('wheres.' . $hash . '_age');
                        $ageDb      = $this->getAge();
                    ...
                    if (empty($rows)) {
                        ...
                        if (!empty($rows)) {
                            ...
                            redis()->set('wheres.' . $hash . '_age', time());
                        }
                    ...
                    if (empty($this->wheres)) {
                        ...
                        if (redis()->exists($cursorQuery)) {
                            $ageCursor  = redis()->get('queries.' . $hashQuery . '_age');
                            $ageCursor  = empty($ageCursor) ? 0 : (int) $ageCursor;
                            $ageDb      = $this->getAge();
                        ...
                        if (!$made) {
                            ...
                            if (!empty($this->results)) {
                                ...
                                redis()->set('queries.' . $hashQuery . '_age', time());
                            }
        ...
        {
            $keys = redis()->keys($this->collection . '*');
        ...
        public function last($object = false, $reset = true)
        {
            ...
            if (true === $object) {
                return $res->count() > 0 ? $res->last(true) : null;
            } else {
                return $res->count() > 0 ? $res->last() : [];
            }
        }
