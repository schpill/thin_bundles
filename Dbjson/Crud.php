<?php
    ...
    class Crud
    {
        ...
        public function __construct(Database $model)
        {
            ...
            $fileConfig = APPLICATION_PATH . DS . 'models' . DS . 'CrudJson' . DS . ucfirst(Inflector::camelize($model->db)) . DS . ucfirst(Inflector::camelize($model->table)) . '.php';
        ...
        public static function instance(Database $model)
        {
            ...
            $has    = Instance::has('DbjsonCrud', $key);

            if (true === $has) {
                return Instance::get('DbjsonCrud', $key);
            } else {
                return Instance::make('DbjsonCrud', $key, new self($model));
            }
        }
        ...
        public function listing($customFields = false)
        {
            ...
            $fieldsSettings = jdb('system', 'datafieldssettings')
            ...
            $tableSettings = jdb('system', 'datatablesettings')
            ->where("table = " . $this->model->table)
            ->where('database = ' . $this->model->db)
            ->where('user_id = ' . auth()->user()->getId())
            ->first();
            ...
            if (request()->getKill() == 1) {
                session('dataTableCrud::' . $this->model->table)
                ->setPage(null)
                ->setOrder(null)
        ...
        public function makeSearch()
        {
            ...
            $queriesRecorded = jdb('system', 'dataquery')
            ->where('user_id = ' . auth()->user()->getId())
            ->where('table = ' . $this->model->table)
            ->where('database = ' . $this->model->db)
            ->exec();
            ...
            if (!empty($where)) {
                $queryRecorded = jdb('system', 'dataquery')
                ->where('user_id = ' . auth()->user()->getId())
                ->where('table = ' . $this->model->table)
                ->where('database = ' . $this->model->db)
