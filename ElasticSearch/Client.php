<?php
    ...
    class Client
    {
        ...
        protected static $_protocols = array(
            ...
            'redis'     => 'ElasticSearch\\Transport\\Redis',
        );
        ...
        public static function connection($config = [])
        {
            ...
            $server = Arrays::is($config['servers']) ? $config['servers'][0] : $config['servers'];
        ...
        public function config($config = null)
        {
            ...
            if (Arrays::is($config)) $this->_config = $config + $this->_config;
        }
        ...
        {
            if (Arrays::is($index)) $index = implode(",", array_filter($index));
        ...
        {
            if (Arrays::is($type)) $type = implode(",", array_filter($type));
        ...
        {
            if (Arrays::is($mapping)) $mapping = new Mapping($mapping);
            ...
            if (isset($type) && !$this->passesTypeConstraint($type)) {
                throw new Exception("Can t create mapping due to type constraint mismatch");
            }
