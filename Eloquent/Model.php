<?php
	...
	abstract class Model
	{
		...
		{
			$this->relating_key = (is_null($foreign_key)) ?
			Inflector::lower(static::model_name($this)) . '_id' :
			$foreign_key;
		...
		public function save()
		{
			...
			if ($this->exists) {
				...
				$success = is_numeric(
					$this->attributes[static::$primary_key] = $this->query->insert_get_id(
						$this->attributes,
						static::$sequence
					)
				);
		...
		public function __call($method, $parameters)
		{
			...
			if (Arrays::in(
				$method,
				array('insert', 'update', 'increment', 'decrement', 'abs', 'count', 'sum', 'min', 'max', 'avg')
			)) {
				return call_user_func_array(
					array(
						$this->query,
